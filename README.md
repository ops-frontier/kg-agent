...
ナレッジグラフエージェントは Github の組織は以下のリポジトリからナレッジグラフを構築し、これを使ったAIエージェントを提供するものである。また、ナレッジグラフに Cypher という言語で直接問い合わせる機能も提供している。ソースコードの解析には Tree-sitter を利用し、ナレッジグラフデータベースには Neo4j を使用している。
...
抽出・変換したデータを **Neo4j** のグラフデータベースへ投入します。
...
* **AI・LLM連携（GraphRAG）**
Vertex AI と接続し、「リポジトリAの変更がリポジトリBにどう影響するか」といった複数コードベースにまたがる回答精度を高めます。
